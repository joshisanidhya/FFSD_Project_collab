...
async function loadAllEvents() {
  let events = [];
  try {
    events = await window.API.events.getAll();
  } catch (e) {
  ...
  const rejected = events.filter(ev => ev.status === 'rejected');
...
async function approveEvent(id) {
  try {
    await window.API.events.update(id, { status: 'approved' });
  } catch (e) {
  ...
  updateStatus(id, 'approved');
  if (window.toast) window.toast('✅ Event approved — all users will now see it.');
}

async function rejectEvent(id) {
  try {
    await window.API.events.update(id, { status: 'rejected' });
  } catch (e) {
    updateStatus(id, 'rejected');
    return;
  }
  updateStatus(id, 'rejected');
  if (window.toast) window.toast('❌ Event rejected.');
}
