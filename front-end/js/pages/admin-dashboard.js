...
function renderOverview() {
    ...
    if (auditEl) auditEl.innerHTML = (_auditLog.slice(0, 4).map(item => `
        <div class="audit-item">
            <div class="audit-action">${esc(item.action)}</div>
            <div class="audit-meta">${esc(item.actor)} · ${new Date(item.createdAt).toLocaleString()}</div>
        </div>
...
function renderAuditLog() {
    ...
    if (!tbody) return;
    tbody.innerHTML = _auditLog.map(item => `
        <tr>
            <td>${esc(item.action)}</td>
            <td>${esc(item.target || '—')}</td>
            <td>${esc(item.actor || 'system')}</td>
    ...
    `).join('');
    if (!_auditLog.length) {
        tbody.innerHTML = '<tr><td colspan="6"><div class="empty-state"><div class="empty-state-text">No audit entries</div></div></td></tr>';
    }
}
...
window.updateConfig = async function (key, value) {
    ...
    try {
        _platformConfig = await window.API.platformConfig.update({ [backendKey]: value });
